...
public class PurchaseView implements Serializable {
    ...
    public String submitEnrollment() {
        ...
        cartView.addEnrollmentItem(sku, qty, mode, contact, deliveryPrefs, describeVariant(sku));
    ...
    public String submitVerification() {
        ...
        VerificationSku sku = getSelectedVerificationSku();
        if (sku == null) {
        ...
        Contact contact = new Contact(email, msisdn);
        DeliveryPrefs deliveryPrefs = new DeliveryPrefs(deliverEmail, deliverSms);
        cartView.addVerificationItem(sku, qty, mode, contact, deliveryPrefs, describeVerificationVariant(sku));
        return "cart?faces-redirect=true";
    }
    ...
    private VerificationSku getSelectedVerificationSku() {
        if (!isVerificationFlow() || selectedSku == null || selectedSku.isBlank()) {
            return null;
        }
        return VerificationSku.bySku(selectedSku).orElse(null);
    }
}
